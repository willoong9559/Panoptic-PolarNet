...
from utils.log import log_writer
...
def main(args):
    ...
    my_model.to(pytorch_device, non_blocking=True)
    ...
    #prepare dataset
    log_writer.print_write_log("dataset_path = " + data_path + '/sequences/')
    ...
    log_writer.print_write_log("Train dataset size:" + len(train_dataset))
    ...
    while epoch < args['model']['max_epoch']:
        ...
        for i_iter,(train_vox_fea,train_label_tensor,train_gt_center,train_gt_offset,train_grid,_,_,train_pt_fea) in enumerate(train_dataset_loader):
            ...
            if global_iter % check_iter == 0:
                ...
                with torch.no_grad():
                    for i_iter_val,(val_vox_fea,val_vox_label,val_gt_center,val_gt_offset,val_grid,val_pt_labels,val_pt_ints,val_pt_fea) in enumerate(val_dataset_loader):
                        val_vox_fea_ten = val_vox_fea.to(pytorch_device, non_blocking=True)
                        val_vox_label = SemKITTI2train(val_vox_label)
                        val_pt_fea_ten = [torch.from_numpy(i).type(torch.FloatTensor).to(pytorch_device, non_blocking=True) for i in val_pt_fea]
                        val_grid_ten = [torch.from_numpy(i[:,:2]).to(pytorch_device, non_blocking=True) for i in val_grid]
                        val_label_tensor=val_vox_label.type(torch.LongTensor).to(pytorch_device, non_blocking=True)
                        val_gt_center_tensor = val_gt_center.to(pytorch_device, non_blocking=True)
                        val_gt_offset_tensor = val_gt_offset.to(pytorch_device, non_blocking=True)
                        ...
                        for count,i_val_grid in enumerate(val_grid):
                            # get foreground_mask
                            for_mask = torch.zeros(1,grid_size[0],grid_size[1],grid_size[2],dtype=torch.bool).to(pytorch_device, non_blocking=True)
                            for_mask[0,val_grid[count][:,0],val_grid[count][:,1],val_grid[count][:,2]] = True
                            # post processing
                            panoptic_labels,center_points = get_panoptic_segmentation(torch.unsqueeze(predict_labels[count], 0),torch.unsqueeze(center[count], 0),\
                ...
                log_writer.print_write_log('Validation per class PQ, SQ, RQ and IoU: ')
                ...
                for class_name, class_pq, class_sq, class_rq, class_iou in zip(unique_label_str,class_all_PQ[1:],class_all_SQ[1:],class_all_RQ[1:],ious[1:]):
                    log_writer.print_write_log('%15s : %6.2f%%  %6.2f%%  %6.2f%%  %6.2f%%' % (class_name, class_pq*100, class_sq*100, class_rq*100, class_iou*100))                              
                # save model if performance is improved
                if best_val_PQ<class_PQ:
                    best_val_PQ=class_PQ
                    torch.save(my_model.state_dict(), model_save_path)
                log_writer.print_write_log('Current val PQ is %.3f while the best val PQ is %.3f' %
                    (class_PQ*100,best_val_PQ*100))               
                log_writer.print_write_log('Current val miou is %.3f'%
                    (miou*100))
                ...
                if start_training:
                    ...
                    log_writer.print_write_log('epoch %d iter %5d, loss: %.3f, semantic loss: %.3f, heatmap loss: %.3f, offset loss: %.3f\n' %
                        (epoch, i_iter, sem_l+hm_l+os_l, sem_l, hm_l, os_l))
                log_writer.print_write_log('%d exceptions encountered during last training\n' %
                    exce_counter)
                exce_counter = 0
            ...
            # training
            try:
                train_vox_fea_ten = train_vox_fea.to(pytorch_device, non_blocking=True)
                train_label_tensor = SemKITTI2train(train_label_tensor)
                train_pt_fea_ten = [torch.from_numpy(i).type(torch.FloatTensor).to(pytorch_device, non_blocking=True) for i in train_pt_fea]
                train_grid_ten = [torch.from_numpy(i[:,:2]).to(pytorch_device, non_blocking=True) for i in train_grid]
                train_label_tensor=train_label_tensor.type(torch.LongTensor).to(pytorch_device, non_blocking=True)
                train_gt_center_tensor = train_gt_center.to(pytorch_device, non_blocking=True)
                train_gt_offset_tensor = train_gt_offset.to(pytorch_device, non_blocking=True)
...
if __name__ == '__main__':
    ...
    log_writer.print_write_log(' '.join(sys.argv))
    log_writer.print_write_log(args)
    main(args)
